...
main = Tk()
main.title("Python Enterprises")
main.geometry("300x200")

#--------Naa dire akong global variables-------
selected_product = None; quantity = ''; total = 0.0; profit = 0.0; remaining_stock = 0; current_date = datetime.now();final_date = current_date.strftime("%Y-%m-%d  %H:%M:%S"); sub_counter = 0; darkMode = False; product_btn = []; quantity_btn = []; save_to_file_counter = 0; passCounter = 3
...
def mainApp():
    global passCounter
    passwordVerify = passwordEntry.get()
    username = usernameEntry.get()
    if username == "Enter username here...":
        messagebox.showwarning("Error!", "You have entered an empty username.")
        return
    
    elif passwordVerify == "Enter password here...":
        messagebox.showwarning("Error!", "You have entered an empty password.")
        return
    
    else:
        if passwordVerify == "admin123":
            main.destroy()
            ...
            def initialize_file():
                ...
                try:
                    ...
                    {"Item": "Freska Tuna", "Inventory count": 50, "Purchase Price": 10.00, "Selling Price": 11.00}
                        ]
            ...
            def totalCalculation():
                ...
                content_Box.insert(END, f"\nTotal Sale: {total}\nProfit: ₱{profit: .2f} as of <{final_date}>\n\n")
            ...
            passCounter -= 1
            messagebox.showerror("Invalid Login!", f"Password did not match! You have {passCounter} tries left.")
            passwordEntry.delete(0, END)
            if passCounter < 1:
                messagebox.showinfo("Wrong Passcode!", "You entered the wrong passcode 3 times. Program will now close.")
                main.destroy()
            return
        

#---------widgets-----------
Label(main, text="Python Enterprises", font=("Arial", 20)).pack()

usernameLabel = Label(main, text="Username", font=("Arial", 12, "bold"))
usernameLabel.pack()

usernameEntry = Entry(main)
usernameEntry.pack()
add_placeholder(usernameEntry, "Enter username here...")

passwordLabel = Label(main, text="Password", font=("Arial", 12, "bold"))
passwordLabel.pack()

passwordEntry = Entry(main)
passwordEntry.pack()
add_placeholder(passwordEntry, "Enter password here...", is_password=True)

loginButton = Button(main,
                     text="Login",
                     font=("Arial", 12, "bold"),
                     command=mainApp,
                     width=8, height=2)
